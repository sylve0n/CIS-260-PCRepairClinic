...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
  ...
  //2. ***************Add part to appropriate table and get the generated**************** //
  $str = "Insert into {$tableName} (";
  foreach ($_POST as $key => $value) {
    if ($key != "tableName") $str .= $key . ", ";
  }
  $str .= "BarCodeID) VALUES (";
  foreach ($_POST as $key => $value) {
    if ($key != "tableName") $str .= "'$value', ";
  }
  $str .= "$bcID)";
...
print "<form method='post' id='frmPart'><table>";
    ...
    print "<input type='button' onclick='saveChanges()' value='Add Part' />";
print "</form>";
 ?>
